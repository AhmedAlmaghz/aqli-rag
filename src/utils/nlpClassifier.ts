/**
 * Lightweight NLP Document Classifier & Metadata Extractor
...
// Multilingual lexicon dictionaries with weighted relevance
...
/**
 * Normalizes text for linguistic NLP matching
 */
...
/**
 * Analyzes language composition (Arabic, English, or Mixed)
 */
 ...
 * Ingests raw document text + title and returns predicted category, confidence, language, and key topical concepts.
 */
export function classifyDocumentNlp(title: string, content: string): NlpClassificationResult {
  const normTitle = normalizeForNlp(title);
  const normContent = normalizeForNlp(content.slice(0, 15000)); // Sample first 15k chars for ultra-fast NLP
  const titleTokens = normTitle.split(/\s+/).filter(w => w.length > 2);
  const contentTokens = normContent.split(/\s+/).filter(w => w.length > 2);
  ...
  for (const [catKey, lexicon] of Object.entries(CATEGORY_LEXICON)) {
    ...
    for (const word of combinedLexicon) {
      ...
      // Check word token exact matches in title
      if (titleTokens.includes(normWord)) {
        score += 2.0;
      }
    }

    // 2. Check content matches (1.0x with saturation)
    let contentHits = 0;
    for (const word of combinedLexicon) {
      const normWord = normalizeForNlp(word);
    ...
    // Sub-linear TF saturation for content hits
  ...
  // If score is negligible (< 1.5), default to general
  if (highestScore < 1.5) {
    bestCategory = 'general';
    highestScore = Math.max(1.0, highestScore);
  }

  // Calculate confidence score using normalized distribution
  const totalScore = Object.values(scores).reduce((a, b) => a + b, 0) || 1;
  const rawConfidence = highestScore / totalScore;
  // Scale confidence pleasantly between 78% and 98%
